/**
 * Netease API 服务（Android 版）
 *
 * 流程与上游 dev 一致：
 *   1) 从 sessions 加载 cookies 注入（内存缓存）
 *   2) 走一层内存响应缓存（2 分钟）
 *   3) 路由到 modules/<name>
...
/** 会变更登录态的接口：响应里若带 set-cookie，才值得写回存储 */
...
export const callNetease = async (
  ...
  const isSessionEndpoint =
    name.startsWith("login") || name === "logout" || name === "register_anonimous";
