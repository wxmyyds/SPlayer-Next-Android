...
export const aesCbcEncrypt = async (
  ...
  const k = await crypto.subtle.importKey("raw", key as BufferSource, { name: "AES-CBC" }, false, [
    "encrypt",
  ]);
...
export const aesGcmEncrypt = async (
  ...
  const k = await crypto.subtle.importKey("raw", key as BufferSource, { name: "AES-GCM" }, false, [
    "encrypt",
  ]);
...
export const rawRsaEncrypt = (data: Uint8Array, pem: string, leftPad: boolean): string => {
  ...
  return modPow(bytesToBig(block), e, n)
    .toString(16)
    .padStart(keyLen * 2, "0");
};
...
export const pkcs1Encrypt = (data: Uint8Array, pem: string): string => {
  ...
  return modPow(bytesToBig(em), e, n)
    .toString(16)
    .padStart(keyLen * 2, "0");
};
