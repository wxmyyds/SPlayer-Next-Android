/**
 * KG 加密与签名工具（Android 版）
 *
 * 算法与上游 dev 完全一致（MD5 / AES-CBC / 裸 RSA / PKCS#1 / 签名拼接），
 * 只是把 node:crypto 换成 shim/webcrypto 的 Web 原语。
 */
...
/**
 * 按 KG 客户端协议执行无填充 RSA 加密（明文左对齐）
 * @param data 待加密对象
 * @returns 十六进制密文
 */
export const rsaEncryptKugou = (data: Record<string, unknown>): string =>
  rawRsaEncrypt(utf8(JSON.stringify(data)), currentSalts().publicKey, false);

/** PKCS#1 v1.5 RSA 加密（设备注册 p 字段） */
export const rsaEncryptKugouPkcs1 = (data: Record<string, unknown>): string =>
  pkcs1Encrypt(utf8(JSON.stringify(data)), currentSalts().publicKey);

/** 设备数据 AES-CBC 密钥/向量：md5hex(key) 前后 16 个 ASCII 字符（与上游一致） */
const deviceKeyIv = (key: string): { key: Uint8Array; iv: Uint8Array } => {
  const digest = cryptoMd5(key);
  return { key: utf8(digest.slice(0, 16)), iv: utf8(digest.slice(16, 32)) };
};

export const encryptKugouDeviceData = (
  data: Record<string, unknown>,
): Promise<{ key: string; content: string }> => {
  const key = randomHexString(3);
  const { key: aesKey, iv } = deviceKeyIv(key);
  return aesCbcEncrypt(aesKey, iv, utf8(JSON.stringify(data))).then((cipher) => ({
    key,
    content: b64Encode(cipher),
  }));
};

export const decryptKugouDeviceData = async (
  content: Uint8Array,
  key: string,
): Promise<unknown> => {
  const { key: aesKey, iv } = deviceKeyIv(key);
  const text = utf8Decode(await aesCbcDecrypt(aesKey, iv, content));
...
export const signatureAndroidParams = (
  ...
  const paramsString = Object.keys(params)
    ...
    .join("");
  const bodyStr = typeof data === "string" ? data : utf8Decode(data);
  return cryptoMd5(
