...
public class NativeHttpPlugin extends Plugin {
    ...
    public void request(PluginCall call) {
        ...
        // 非法 URL/header（如服务端透传的坏地址）在 OkHttp 构造期同步抛 IAE：
        // 插件方法抛异常会被 Bridge 重新抛进 HandlerThread 直接崩进程，必须拦住转 reject
        ...
        try {
            ...
            if ("GET".equals(method) || "HEAD".equals(method)) {
                ...
                } else if (call.getBoolean("bodyIsBase64", false)) {
                    // 二进制 body 以 base64 过桥（JS 侧标记），须还原原始字节
                    byte[] raw = Base64.decode(bodyStr, Base64.NO_WRAP);
                    requestBody = RequestBody.create(raw, mediaType);
                } else {
