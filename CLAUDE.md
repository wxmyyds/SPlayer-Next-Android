...
> 上游是桌面端（Electron + Windows/macOS/Linux），本 fork 目标是安卓端。本文件与 `AGENTS.md`
...
- 本文件与 `AGENTS.md`、`DEVELOPMENT.md` 为 fork 所有，合并时保留我方版本。
...
Don't edit `auto-imports.d.ts`, `components.d.ts`, `native/*/index.d.ts` — regenerated by tooling.（P2 性能补丁给 `index.d.ts` 加行是例外，随补丁走，不直接提交。）
